...
class SBPB_WP_Import extends WP_Importer
{
	...
	public function import_start($file)
	{
		...
		if (!in_array($file_type['ext'], array('csv', 'xml', 'txt') ) ) {
	...
	public function import_end()
	{
		...
		echo '<p>' . esc_html__( 'Remember to update the passwords and roles of imported users.', 'popup-builder' ) . '</p>';
	...
	public function handle_upload()
	{
		...
		add_filter('upload_dir', [$this, 'sgpb_setCustomUploadPopupsPathImport']);

		$file = wp_import_handle_upload();

	   remove_filter('upload_dir', [$this, 'sgpb_setCustomUploadPopupsPathImport']);
	...
	public function author_select( $n, $author )
	{
		...
		echo ' ' . wp_dropdown_users(
			array(
				...
				'multi'           => true,
				'show_option_all' => __( '- Select -', 'popup-builder' ),
	...
	public function get_author_mapping()
	{
		...
		*/ 		
		...
		$imported_authors = isset($_POST['imported_authors']) ? array_map('sanitize_text_field', wp_unslash( $_POST['imported_authors'] ) ) : [];
	...
	public function fetch_remote_file($url, $post)
	{
		...
		if ( ! $tmp_file_name ) {
			return new WP_Error( 'import_no_file', __( 'Could not create temporary file.', 'popup-builder' ) );
		}
		...
		if ( is_wp_error( $remote_response ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Request failed due to an error: %1$s (%2$s)', 'popup-builder' ),
		...
		if ( 200 !== $remote_response_code ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Remote server returned the following unexpected result: %1$s (%2$s)', 'popup-builder' ),
		...
		if ( ! isset( $headers['content-encoding'] ) && isset( $headers['content-length'] ) && $filesize !== (int) $headers['content-length'] ) {
			wp_delete_file( $tmp_file_name );
			return new WP_Error( 'import_file_error', __( 'Downloaded file has incorrect size', 'popup-builder' ) );
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			...
			return new WP_Error( 
			    ...
			    sprintf( 
			        ...
			        __( 'Remote file is too large, limit is %s', 'popup-builder' ), 
		...
		if ( ( ! $type || ! $ext ) && ! current_user_can( 'unfiltered_upload' ) ) {
			return new WP_Error( 'import_file_error', __( 'Sorry, this file type is not permitted for security reasons.', 'popup-builder' ) );
		}
		...
		if ( ! $move_new_file ) {
			wp_delete_file( $tmp_file_name );
			return new WP_Error( 'import_file_error', __( 'The uploaded file could not be moved', 'popup-builder' ) );
		}
		...
		if ( $wp_filesystem && $wp_filesystem->exists( $new_file ) ) {
	...
	{
		echo '<div class="wrap">';
		echo '<h2>' . esc_html( __('Import Popups', 'popup-builder') ). '</h2>';
	 ...
	 * @return string|null File extension if available, or null if not found.
	 */
	public function sgpb_setCustomUploadPopupsPathImport( $dir )
    {
        remove_filter('upload_dir', [$this, 'sgpb_setCustomUploadPopupsPathImport']);

        $dir = wp_upload_dir();

        $path = $dir['basedir'].DIRECTORY_SEPARATOR.'importsgpb' ;
        $url = $dir['baseurl'].'/'.'importsgpb';
