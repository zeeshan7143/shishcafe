					...
					if( $imgSzAlternatives -> srcTpl === null )
					{
						if( in_array( $fileType, array( 'jpe','jpg','jpeg','png','gif','bmp', 'webp','avif' ) ) )
						{
							...
							if( Gen::GetArrField( $settImg, array( 'redirOwn' ), false ) )
								$imgSzAlternatives -> srcTpl = Net::UrlDeParse( array( 'path' => $ctxProcess[ 'siteRootUri' ] . '/', 'query' => Image_MakeOwnRedirUrlArgs( $fileOrigRelPath, substr( $fileTpl, strlen( $ctxProcess[ 'siteCacheRootDir' ] . '/s/' ) ) ) ), Net::URLPARSE_F_PRESERVEEMPTIES );
							else
								$imgSzAlternatives -> srcTpl = Net::UrlDeParse( array( 'path' => $ctxProcess[ 'siteRootUri' ] . '/' . $fileOrigRelPath, 'query' => Image_MakeAiUrlArgs( Gen::GetArrField( $imgSrc -> srcInfo, array( 'args' ), array() ), $fileOrigRelPath, substr( $fileTpl, strlen( $ctxProcess[ 'siteCacheRootDir' ] . '/s/' ) ) ) ), Net::URLPARSE_F_PRESERVEEMPTIES );
...
function _Images_ProcessItemLazy_Finish( &$ctxProcess, $doc, $settImg, $item, $imgSrc, $modeLazy )
{
	...
	if( Gen::GetArrField( $settImg, array( 'lazy', 'own' ), false ) )
	{
		...
		if( $srcset )
		{
			...
			foreach( $srcItems as &$srcItem )
			{
				$imgSrcSub = new ImgSrc( $ctxProcess, html_entity_decode( $srcItem[ 0 ] ) );
				$imgSrcSub -> Init();
