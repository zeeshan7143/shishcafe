...
function OnAdminApi_SelfDiag_VendorSrv( $args )
{
	...
	if( ( Gen::GetArrField( $sett, array( 'cache', 'procEngn' ), 1 ) & 2 ) )
	{
		...
		{
			$procUrl = Gen::StrReplaceKeyed( array( '{FN}' => 'TestConn', '{EP}' => rawurlencode( PluginLic::GetEpName() ), '{V}' => rawurlencode( '2.29.7' ), '{CB}' => rawurlencode( Net::UrlAddArgs( Wp::GetSiteRootUrl( '/', false ), array( 'seraph_accel_at' => 'ORC', 'f' => 'TestConn', '_n' => Gen::GetNonce( 'ORC', GetSalt(), 300 ) ) ) ) ), $procUrl );
